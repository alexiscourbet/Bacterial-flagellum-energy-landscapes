...
class PoseResnumIndex(object):
    ...
    def clashes(self, pose):
        for res in pose:
            for ia in range(1, 5):  #, res.nheavyatoms() + 1):
                xyz = res.xyz(ia)
...
def eval_contacts_pair(rep_pose,dock_gen,contact_cutoff):
   good_matches=[]
   contact_hist={}
   checker=PoseResnumIndex(rep_pose, clash_dis=2.5)
   #print(rep_pose, checker)
   ntries=0
...
def eval_rama(res_type,phi,psi,sf):
    score=sf.eval_rama_score_residue(res_type,phi,psi)
    return score

# have to wait for python 3 for this
def calc_repeat_protein_params_ws(input_file, offset):
 verbose=1
 p=Pose()
 ...
 for line in rep_file:
     ...
     DHR_file=line[0] #struct_dir+line[0]
     DHR=line[0]
...
def rotate_around_x(axis,angle,pose):
#    deg=2.*math.pi/360.
    r_axis=axis.to_rosetta()
#    transform=rosetta.numeric.rotation_matrix_double_t(r_axis,angle*deg)
#    v_axis=Vec(axis)
    t=rotation_matrix_degrees(Vec(1,0,0),angle)
    for ir in range(1, pose.size() + 1):
        for ia in range(1, pose.residue(ir).natoms() + 1):
            aid = rosetta.core.id.AtomID(ia, ir)
            oldxyz = Vec(pose.xyz(aid))
...
def translate_along_z(dist, pose):
    for ir in range(1, pose.size() + 1):
        for ia in range(1, pose.residue(ir).natoms() + 1):
            ...
            #print (pose.xyz(aid))
            #print (Vec(0.,0.,10))
            newxyz = Vec(pose.xyz(aid)) + Vec(0.,0.,float(dist))
            #print(newxyz)
            pose.set_xyz(aid, newxyz.to_rosetta())
    return pose
...
def center_com_x(p):
   ...
   for ir in range(1,p.size()+1):
        for ia in range(1,p.residue(ir).natoms() + 1):
            ...
            n=n+1
            x_tot=x_tot+x
   x_vec=Vec(x_tot/float(n),0.,0.)
#   print('z offset %s'%z_vec)
   for ir in range(1, p.size() + 1):
        for ia in range(1, p.residue(ir).natoms() + 1):
            aid = rosetta.core.id.AtomID(ia, ir)
            newxyz = Vec(p.xyz(aid))-x_vec
            p.set_xyz(aid, newxyz.to_rosetta())
   return p
...
def center_com_y(p):
   ...
   for ir in range(1,p.size()+1):
        for ia in range(1,p.residue(ir).natoms() + 1):
            ...
            n=n+1
            y_tot=y_tot+y
   y_vec=Vec(0.,y_tot/float(n),0.)
#   print('z offset %s'%z_vec)
   for ir in range(1, p.size() + 1):
        for ia in range(1, p.residue(ir).natoms() + 1):
            aid = rosetta.core.id.AtomID(ia, ir)
            newxyz = Vec(p.xyz(aid))-y_vec
            p.set_xyz(aid, newxyz.to_rosetta())
   return p
